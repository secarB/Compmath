...
def solve(y, x_from, x_to, h):
    ...
    print(
        f'We calculate the amounts:\nSX = {fSX}, SXX = {fSXX}, SY = {fSY}, SXY = {fSXY}')
    print('We get a system of linear equations:')
    print(f'{fSXX}a + {fSX}b = {fSXY}')
    print(f'{fSX}a + {num}b = {fSY}')
    print(f'Det: {d = :.3f}, {d1 = :.3f}, {d2 = :.3f}')
    print('Solving the system, we obtain the values of the coefficients:')
    a, b = d1 / d, d2 / d
    fa, fb = map(format_float, (a, b))
    print(f'a = {format_float(d1)} / {format_float(d)} = {fa}')
    print(f'b = {format_float(d2)} / {format_float(d)} = {fb}')
    print(f'Check. '
          f'Calculate the values of the function P(X) = {fa}x + {fb}')
    ...
    print(f'b = {format_float(d2)} / {format_float(d)} = {fb}')
    print(f'Check '
          f'Calculate the values of the function P(X) = {a0:.3f}x^2 + {a1:.3f}x + {a2:.3f}')
